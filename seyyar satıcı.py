...
def crossover(parent1, parent2):
    child = [None] * num_cities #8 tane boş şehirden oluşan bir liste oluştur
    ...
    parent2_ptr = 0
    for i in range(num_cities):#bebeğin her bir şehrini (genini) kontrol et
        if child[i] is None:
            # Çocuğa henüz eklenmemiş bir şehir bulana kadar Ebeveyn 2'de ilerle
            while parent2[parent2_ptr] in child:#sadece çocuğa eklenmemiş bir şehir bulana kadar ebeveyn2’de ilerler
                
                parent2_ptr += 1 #sonraki şehre geç
            child[i] = parent2[parent2_ptr] #ebeğin boş yerine ebeveyn 2'den şehri ekle
            
    return child

# --- Adım 6: Mutasyon (Takas Mutasyonu) ---
def mutate(tour, mutation_rate):
    if random.random() < mutation_rate: # 0 dahil 1 hariç arasında verilen rastgele sayının
        #Bu aralık tüm olasılıkların sadece %[mutation_rate]’i mutasyon geçirir → mutasyon nadiren gerçekleşir
...
if __name__ == '__main__': 
    # Parametreler
    POPULATION_SIZE = 100 # Popülasyondaki birey (tur) sayısı
    GENERATIONS = 500     # Algoritmanın kaç nesil çalışacağı
    MUTATION_RATE = 0.02  # Mutasyon olasılığı (%2)
    ELITISM_SIZE = 5      # En iyi kaç bireyin korunacağı
